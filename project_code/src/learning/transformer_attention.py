...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, input_shapes):
        ...
        self.query_layer = weight_norm(nn.Linear(self.qa_channels, self.qa_channels, bias=False, device = device))
        self.key_layer = weight_norm(nn.Linear(self.qa_channels, self.qa_channels, bias=False, device = device))
        self.value_layer = weight_norm(nn.Linear(self.ma_channels, self.ma_channels, bias=False, device = device))

        self.output_layer = weight_norm(nn.Linear(self.qa_channels, self.qa_channels, bias=False, device = device))
